...
(`gcard`'s soundness against `Query.run` — rows fit the symbolic bound
at the environment's own sizes — is the planned successor, pending the
instance-coherence groundwork.) -/
